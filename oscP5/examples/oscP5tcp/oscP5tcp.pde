...
void mousePressed() {
  ...
  oscP5tcpClient.send("/test", new Object[] {
    new Integer(1)
  }
  );
}
...
void oscEvent(OscMessage theMessage) {
  ...
  else if (theMessage.checkAddrPattern("/AFF")) {
    //float test = (float)theMessage.arguments();
    
    
    String thisvalue = theMessage.get(0).stringValue();
    Float thisfloat = theMessage.get(1).floatValue();

  } 
  else if (theMessage.checkAddrPattern("/COUNT")) {
    //println(theMessage.arguments());
  }
}
