/**
  ...
  Converted to a droppable tab by MZA
...
void readEmotiv(String teamName, OscMessage theOscMessage){
  ...
  for(Door d: doors)
  {
      ...
      if(d.teamA)
      {
          ...
          floor.control.get(2).y =floor.start.get(2).y + statesValuesA[0]*500;
      }
      else
      {
          d.p.y = d.p0.y - statesValuesB[0]*100;
          floor.control.get(0).y =floor.start.get(0).y - statesValuesB[0]*100;
          floor.control.get(3).y =floor.start.get(3).y - statesValuesB[0]*100;
